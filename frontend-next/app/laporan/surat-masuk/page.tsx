...
// Lazy load Calendar to reduce initial bundle
...
export default function LaporanSuratMasukPage() {
  const [rows, setRows] = useState<SuratMasukRow[]>([]);
  ...
  const breadcrumbs: ReactNode = (
    <BreadcrumbList>
      ...
      <BreadcrumbSeparator />
      <BreadcrumbItem>
        <BreadcrumbLink href="/laporan/surat-masuk">Surat Masuk</BreadcrumbLink>
      </BreadcrumbItem>
    </BreadcrumbList>
  );
  ...
  const loadRows = async (page = currentPage, perPage = itemsPerPage) => {
    ...
    try {
      ...
      const res = await apiFetch<any>(`/api/v1/surat-masuk?${params.toString()}`, { auth: true });
      const data: SuratMasukRow[] = res?.data ?? [];
      setRows(data);
    ...
    >
      ...
      <SidebarInset>
        ...
        <div className="flex flex-1 flex-col py-4 px-4 lg:px-6">
          <h1 className="text-2xl font-bold mb-6">Data Laporan Surat Masuk</h1>
              ...
              <CardContent className="pt-6">
                <div className="flex flex-col space-y-4">
                  ...
                  <div className="flex space-x-2">
                    ...
                    <Button variant="secondary" onClick={() => { setStartDate(undefined); setEndDate(undefined); loadRows(1, itemsPerPage); }}>Clear</Button>
                    <Button variant="outline" onClick={() => {
                      if (!startDate || !endDate) { toast.error('Pilih tanggal awal dan akhir'); return; }
                      if (startDate > endDate) { toast.error('Tanggal awal tidak boleh lebih besar dari tanggal akhir'); return; }
                      const url = new URL(`${getApiBaseUrl()}/api/v1/laporan/surat-masuk`);
                      url.searchParams.set('tgl_awal', format(startDate, 'yyyy-MM-dd'));
                      url.searchParams.set('tgl_akhir', format(endDate, 'yyyy-MM-dd'));
                      // open with token
                      const token = localStorage.getItem('token');
                      if (token) {
                        // For file downloads with auth, open in new tab with token in query if backend allows, else we can proxy. For now, open URL (assumes Sanctum token accepted via Bearer cookie or token else handle backend).
          ...
          <Card>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center mb-4 space-x-2">
                ...
                <Select value={itemsPerPage.toString()} onValueChange={(value) => { const per = Number(value); setItemsPerPage(per); loadRows(1, per); }}>
                ...
                <div className="space-y-4">
                  ...
                  <div className="overflow-x-auto">
                    <Table>
                      <TableHeader>
                        <TableRow>
                          {['No','No Agenda','No Surat','Perihal','Tgl Surat','Penerima','Tgl Terima','Pengirim','Dibaca'].map((_, i) => (
                            <TableHead key={i}>
                ...
                <>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead className="w-[80px] min-w-[80px]">No Agenda</TableHead>
                          ...
                          <TableHead className="w-[150px] min-w-[150px]">Penerima</TableHead>
                          <TableHead className="w-[80px] min-w-[80px]">Tgl Terima</TableHead>
                          <TableHead className="w-[150px] min-w-[150px]">Pengirim</TableHead>
                          <TableHead className="w-[120px] min-w-[120px]">Dibaca</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {rows.length > 0 ? (
                          rows.map((row, index) => (
                            <TableRow key={index}>
                              ...
                              <TableCell className="max-w-[80px] truncate">{row.id}</TableCell>
                              <TableCell className="max-w-[120px] truncate">{row.document?.nomor_surat}</TableCell>
                              <TableCell className="max-w-[200px] truncate">{row.document?.perihal}</TableCell>
                              <TableCell className="max-w-[80px] truncate">{row.document?.tanggal_surat}</TableCell>
                              <TableCell className="max-w-[150px] truncate">{row.asal_surat}</TableCell>
                              <TableCell className="max-w-[80px] truncate">{row.tanggal_diterima}</TableCell>
                              <TableCell className="max-w-[150px] truncate">{row.document?.nomor_surat ? '—' : '-'}</TableCell>
                              <TableCell>
                                <span className={cn('px-3 py-1 rounded-full text-xs', row.status === 'Dibaca' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700')}>{row.status}</span>
                              </TableCell>
                            </TableRow>
                          ))
                        ) : (
                  ...
                  <div className="flex justify-between items-center mt-4">
                    ...
                    <Pagination>
                      <PaginationContent>
                        <PaginationItem>
                          <PaginationPrevious href="#" onClick={(e) => { e.preventDefault(); if (currentPage > 1) loadRows(currentPage - 1, itemsPerPage); }} className={cn({ 'pointer-events-none opacity-50': currentPage === 1 })} />
                        </PaginationItem>
                        {lastPage > 0 && Array.from({ length: lastPage }, (_, i) => (
                          <PaginationItem key={i}>
                            <PaginationLink href="#" isActive={i + 1 === currentPage} onClick={(e) => { e.preventDefault(); if (i + 1 !== currentPage) loadRows(i + 1, itemsPerPage); }}>
                              {i + 1}
                            </PaginationLink>
                          </PaginationItem>
                        ))}
                        <PaginationItem>
                          <PaginationNext href="#" onClick={(e) => { e.preventDefault(); if (currentPage < lastPage) loadRows(currentPage + 1, itemsPerPage); }} className={cn({ 'pointer-events-none opacity-50': currentPage >= lastPage })} />
                        </PaginationItem>
                      </PaginationContent>
                    </Pagination>
                  </div>
